...
typedef struct catalogoL {
    LivroL *cabeca;   // Ponteiro para o primeiro nó da lista (cabeça)
} catalogoL;



// Cria um nó para a lista de empréstimos fornecendo um livro
LivroL *livroL_init(char *nome, char *autor) {
    LivroL *atual = malloc(sizeof(LivroL)); // Aloca memória para a estrutura Livro
...
catalogoL *catalogoL_init() {
    catalogoL *cat = malloc(sizeof(catalogoL)); // Aloca memória para a estrutura catalogo
...
bool compara_livros(LivroL *l1, LivroL *l2){
...
void empstt_append(catalogoL *l, LivroL* livro) {
...
void empstt_pop(catalogoL *l, LivroL *livro) {
...
void empstt_status(catalogoL *l, LivroL *livro) {
...
int empstt_pegarlivro(catalogoL *l, LivroL* livro){
...
int empstt_devolverlivro(catalogoL *l, LivroL *livro){
